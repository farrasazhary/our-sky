...
export function ImageViewerModal({ isOpen, onClose, memory, onUpdate }: ImageViewerModalProps) {
  if (!memory) return null

  const authorName = memory.authorName || "Pasangan"

  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[200] flex flex-col justify-between p-6 overflow-y-auto no-scrollbar">
          {/* Header */}
          <div className="flex justify-between items-center text-white z-10 pt-4">
            ...
            <button onClick={onClose} className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-white shrink-0">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <motion.div 
            ...
            className="my-auto relative max-h-[50vh] flex items-center justify-center overflow-hidden rounded-2xl border border-white/10 shadow-2xl my-4"
          >
            <img src={memory.img} alt={memory.title} className="w-full h-full object-contain" />
          </motion.div>

          {/* Caption & Reaction Footer */}
          <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-2xl text-white mb-4 space-y-3">
