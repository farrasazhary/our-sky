...
export function NotificationToast() {
  ...
  const handleToastClick = () => {
    const target = toast.targetUrl || (toast.type === "HEARTBEAT" ? "/dashboard" : "/notifications")
    setToast(null)
    navigate(target)
  }

  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 max-w-md w-[92%] z-50 pointer-events-auto cursor-pointer">
      <AnimatePresence>
        <motion.div
          ...
          className="bg-surface/95 backdrop-blur-xl border border-primary/40 shadow-2xl rounded-2xl p-4 flex items-center justify-between gap-3 text-left hover:border-primary/70 transition-colors"
        >
